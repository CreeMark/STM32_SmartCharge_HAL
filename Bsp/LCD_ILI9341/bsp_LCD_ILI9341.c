...
void LCD_Init(void)
{
    ...
    // 使能各端口时钟
    // BL引脚
    if (LCD_BL_GPIO == GPIOA)  RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;     // 使能GPIO：GPIOA
    if (LCD_BL_GPIO == GPIOB)  RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;     // 使能GPIO：GPIOB
    if (LCD_BL_GPIO == GPIOC)  RCC->APB2ENR |= RCC_APB2ENR_IOPCEN;     // 使能GPIO：GPIOC
    if (LCD_BL_GPIO == GPIOD)  RCC->APB2ENR |= RCC_APB2ENR_IOPDEN;     // 使能GPIO：GPIOD
    if (LCD_BL_GPIO == GPIOE)  RCC->APB2ENR |= RCC_APB2ENR_IOPEEN;     // 使能GPIO：GPIOE
    if (LCD_BL_GPIO == GPIOF)  RCC->APB2ENR |= RCC_APB2ENR_IOPFEN;     // 使能GPIO：GPIOF
    if (LCD_BL_GPIO == GPIOG)  RCC->APB2ENR |= RCC_APB2ENR_IOPGEN;     // 使能GPIO：GPIOG
    // RD引脚
    if (LCD_BL_GPIO == GPIOA)  RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;     // 使能GPIO：GPIOA
    if (LCD_BL_GPIO == GPIOB)  RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;     // 使能GPIO：GPIOB
    if (LCD_BL_GPIO == GPIOC)  RCC->APB2ENR |= RCC_APB2ENR_IOPCEN;     // 使能GPIO：GPIOC
